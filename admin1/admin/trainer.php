    ...
    <div class="container-fluid">
        <div class="col-md-14">
            <div class="float-right">
                <a href="addtrainer.php" class="btn btn-success">Add +</a>                
                <br><br><br>
            </div>
            <div class="card">
                ...
                <form action="" method="get" class="form-inline float-right">
                    <div class="input-group">
                        ...
                        <?php if (isset($_GET['search'])) { ?>
                            <a href="trainer.php" class="btn btn-default">Clear Search</a>
                        <?php } ?>
                    </div>
                </form>
            </div>
                ...
                <div class="card-body">
                    <div class="table-responsive ps">
                        <table class="table tablesorter" id="page1">
                            ...
                            <tbody>
                            <?php 
                                while (list($id, $name, $image, $contact, $city, $address, $experience, $bio, $created_at, $status) = mysqli_fetch_array($result)) {
                                    echo "<tr>
                                        <td>$id</td>
                                        <td>$name</td>
                                        <td><img src='assets/img/$image' style='width:50px; height:50px; border:groove #000'></td>
                                        <td>$contact</td>
                                        <td>$city</td>
                                        <td>$address</td>
                                        <td>$experience</td>
                                       ...
                                       // Toggle icon for status (Active/Inactive)
                                       if ($status == "Active") {
                                        echo "<a href='trainer.php?id=$id&status=$status&action=toggle_status' title='Disable' style='margin-right: 15px;'>
                                                <i class='fa fa-toggle-on fa-lg text-success'></i>
                                              </a>";
                                    } else {
                                        echo "<a href='trainer.php?id=$id&status=$status&action=toggle_status' title='Enable'>
                                                <i class='fa fa-toggle-off fa-lg text-danger'></i>
                                              </a>";
                                    }

                                    // Add the Edit and Delete buttons
                                    echo " <a href='edittrainer.php?id=$id' class='text-primary' title='Edit'  style='margin-right: 15px;'>
                                                <i class='fa fa-edit fa-lg'></i>
                                            </a>
                                            <a href='trainer.php?id=$id&action=delete' class='text-danger' title='Delete' onclick='return confirm(\"Are you sure you want to delete this contact?\")'>
