                            ...
                            <tbody>
                                ...
                                while (list($id, $name, $description, $img, $created_at, $status) = mysqli_fetch_array($result)) {
                                    echo "<tr>
                                            ...
                                            if ($status == "Active") {
                                                echo "<a href='classes.php?id=$id&status=$status&action=toggle_status' title='Disable' style='margin-right: 15px;'>
                                                        <i class='fa fa-toggle-on fa-lg text-success'></i>
                                                      </a>";
                                            } else {
                                                echo "<a href='classes.php?id=$id&status=$status&action=toggle_status' title='Enable' style='margin-right: 15px;'>
                                                        <i class='fa fa-toggle-off fa-lg text-danger'></i>
                                                      </a>";
                                            }

                                    // Add the Edit and Delete buttons
                                    echo " <a href='editclasses.php?id=$id' class='text-primary' title='Edit'  style='margin-right: 15px;'>
                                                <i class='fa fa-edit fa-lg'></i>
                                            </a>
                                            <a href='classes.php?id=$id&action=delete' class='text-danger' title='Delete' onclick='return confirm(\"Are you sure you want to delete this contact?\")'>
